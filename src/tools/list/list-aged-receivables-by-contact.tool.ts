...
const ListAgedReceivablesByContact = CreateXeroTool(
  ...
  async ({ contactId, reportDate, invoicesFromDate, invoicesToDate }, _extra, xero) => {
    const response = await listXeroAgedReceivablesByContact(xero, contactId, reportDate, invoicesFromDate, invoicesToDate);
